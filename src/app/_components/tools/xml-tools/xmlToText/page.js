...
const XmlToTextConverter = () => {
    ...
    const [textData, setTextData] = useState(null); // State to hold parsed text data
    ...
    const jsonToText = (json) => {
        try {
            const formatObject = (obj, indent = "") => {
                ...
                for (const key in obj) {
                    if (typeof obj[key] === "object" && obj[key] !== null) {
                        text += `${indent}${key}:\n`;
                        text += formatObject(obj[key], indent + "  ");
                    } else {
            ...
            throw new Error("Error converting JSON to Text.");
        }
    };
    ...
    const handleInputChange = (e) => {
        ...
        if (!xml.trim()) {
            ...
            setErrorDetail("");
            setShowError(false);
            setTextData(null); // Clear the text data
            return;
        }
        ...
        xmlToJson(xml)
            .then((parsedData) => {
                const text = jsonToText(parsedData); // Convert JSON to Text
                setTextData(text); // Set the parsed text data
                setErrorMessage("");
                setErrorLine(null);
                setErrorDetail("");
                setShowError(false);
                ...
                <div className="grid grid-cols-2 w-full gap-5">
                    ...
                    <div className="border border-gray-300 rounded-lg overflow-hidden">
                        <div className="bg-gray-800 p-2 flex justify-end space-x-4">
                            ...
                            <span onClick={() => handleCopy(textData || "")} className="cursor-pointer text-white">
                                <Copy size={25} />
                            </span>
                        ...
                        <div className="flex">
                            ...
                            >
                                {textData ? (
                                    ...
                                    <div className="text-gray-500">Text will be generated after conversion.</div>
                                )}
                            </div>
                        </div>
                    </div>
...
export default XmlToTextConverter;