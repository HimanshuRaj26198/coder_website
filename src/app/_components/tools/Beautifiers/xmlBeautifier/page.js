    ...
    const XmlBeautifier = () => {
        ...
        const handleInputChange = (e) => {
            ...
            if (!xml.trim()) {
                ...
                setErrorLine(null);
                setShowError(false);
                setBeautifiedXml(""); // Clear the beautified XML
                return;
            }

            try {
                // Beautify the XML
                const result = beautifyXml(xml);
                setBeautifiedXml(result);
                setErrorMessage("");
                setErrorLine(null);
                setShowError(false);
            } catch (error) {
        ...
        const handleBeautify = () => {
            if (!inputXml.trim()) {
                setErrorMessage("No XML code to beautify.");
                setShowError(true);
                return;
            }

            try {
                const result = beautifyXml(inputXml);
                setBeautifiedXml(result);
            } catch (error) {
                setErrorMessage(error.message);
                const match = error.message.match(/at position (\d+)/);
        ...
        const handleDownload = () => {
            if (!beautifiedXml.trim()) {
                setErrorMessage("No beautified XML to download.");
                setShowError(true);
                return;
            }
                    ...
                    <div className="grid grid-cols-2 w-full gap-5">
                        ...
                        <div className="border border-gray-300 rounded-lg overflow-hidden">
                            <div className="bg-gray-800 p-2 flex justify-end space-x-4">
                                ...
                                <span onClick={() => handleCopy(beautifiedXml || "")} className="cursor-pointer text-white">
                                    <Copy size={25} />
                                </span>
                            ...
                            <div className="flex">
                                <div
                                    ...
                                    ref={outputLineRef}
                                >
                                    <pre>{getLineNumbers(beautifiedXml)}</pre>
                                </div>
                                <textarea
                                    ref={outputTextRef}
                                    onScroll={handleScroll}
                                    className="w-full p-2 bg-gray-900 text-white outline-none resize-none"
                                    style={{ height: 400 }}
                                    placeholder="Beautified XML will appear here..."
                                    value={beautifiedXml}
                                    readOnly
                                />
                            </div>
