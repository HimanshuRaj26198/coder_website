...
const XmlToHtmlConverter = () => {
    ...
    const [htmlData, setHtmlData] = useState(null); // State to hold parsed HTML data
    ...
    const jsonToHtml = (json) => {
        try {
            const formatObject = (obj) => {
                ...
                for (const key in obj) {
                    if (typeof obj[key] === "object" && obj[key] !== null) {
                        html += `<div><strong>${key}:</strong> ${formatObject(obj[key])}</div>`;
                    } else {
            ...
            throw new Error("Error converting JSON to HTML.");
        }
    };
    ...
    const handleInputChange = (e) => {
        ...
        if (!xml.trim()) {
            ...
            setErrorDetail("");
            setShowError(false);
            setHtmlData(null); // Clear the HTML data
            return;
        }
        ...
        xmlToJson(xml)
            .then((parsedData) => {
                const html = jsonToHtml(parsedData); // Convert JSON to HTML
                setHtmlData(html); // Set the parsed HTML data
                setErrorMessage("");
                setErrorLine(null);
                setErrorDetail("");
                setShowError(false);
                ...
                <div className="grid grid-cols-2 w-full gap-5">
                    ...
                    <div className="border border-gray-300 rounded-lg overflow-hidden">
                        <div className="bg-gray-800 p-2 flex justify-end space-x-4">
                            ...
                            <span onClick={() => handleCopy(htmlData || "")} className="cursor-pointer text-white">
                                <Copy size={25} />
                            </span>
...
export default XmlToHtmlConverter;