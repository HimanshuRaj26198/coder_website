...
const XmlToJavaConverter = () => {
    ...
    const [javaData, setJavaData] = useState(null); // State to hold parsed Java data
    ...
    const xmlToJson = async (xml) => {
        try {
            ...
            throw new Error(`Invalid XML format. ${lineNumber ? `Error at line: ${lineNumber}` : ""}`);
        }
    };

    // Convert JSON to Java
    const jsonToJava = (json) => {
        try {
            const generateJavaClass = (obj, className = "Root") => {
                let javaCode = `public class ${className} {\n`;

                for (const key in obj) {
                    if (typeof obj[key] === "object" && obj[key] !== null) {
                        // Handle nested objects
                        javaCode += `    public ${key} ${key};\n`;
                        javaCode += generateJavaClass(obj[key], key);
                    } else {
                        // Handle primitive types
                        javaCode += `    public String ${key};\n`;
                    }
                }
            ...
            return generateJavaClass(json);
        } catch (error) {
            throw new Error("Error converting JSON to Java.");
        }
    };
    ...
    const handleInputChange = (e) => {
        ...
        if (!xml.trim()) {
            ...
            setErrorDetail("");
            setShowError(false);
            setJavaData(null); // Clear the Java data
            return;
        }
        ...
        xmlToJson(xml)
            .then((parsedData) => {
                const javaCode = jsonToJava(parsedData); // Convert JSON to Java
                setJavaData(javaCode); // Set the parsed Java data
                setErrorMessage("");
                setErrorLine(null);
                setErrorDetail("");
                setShowError(false);
    ...
    const handleConvert = () => {
        try {
            ...
            // Create a Blob from the Java content
            const blob = new Blob([javaData], { type: "text/plain" });
                ...
                <div className="grid grid-cols-2 w-full gap-5">
                    ...
                    <div className="border border-gray-300 rounded-lg overflow-hidden">
                        <div className="bg-gray-800 p-2 flex justify-end space-x-4">
                            ...
                            <span onClick={() => handleCopy(javaData || "")} className="cursor-pointer text-white">
                                <Copy size={25} />
                            </span>
                        ...
                        <div className="flex">
                            ...
                            >
                                {javaData ? (
                                    ...
                                    <div className="text-gray-500">Java will be generated after conversion.</div>
                                )}
                            </div>
                        </div>
                    </div>
...
export default XmlToJavaConverter;