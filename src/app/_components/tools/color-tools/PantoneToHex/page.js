...
const PantoneToHex = () => {
    ...
    const [hex, setHex] = useState(pantoneToHexMap['PANTONE 100']); // Default HEX value
    ...
    // Update HEX when pantone changes
    useEffect(() => {
        const newHex = pantoneToHexMap[pantone];
        if (!newHex) {
        ...
        setErrorMessage('');
        setHex(newHex);
        setColorPickerColor(new TinyColor(newHex).toRgb());
    }, [pantone]);
    ...
    const handleColorChange = (color) => {
        ...
        const newHex = new TinyColor({ r, g, b }).toHexString();

        // Find the matching Pantone color
        const matchingPantone = Object.keys(pantoneToHexMap).find((key) => pantoneToHexMap[key] === newHex) || 'Custom Color';
                ...
                <div className="column is-12">
                    ...
                    <div className="columns">
                        ...
                        <div className="column">
                            {/* <h1 className="title has-text-centered">Pantone to HEX Converter</h1> */}
                        </div>
