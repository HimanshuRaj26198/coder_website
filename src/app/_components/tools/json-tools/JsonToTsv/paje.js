...
const JsonToTsvConverter = () => {
    ...
    const [convertedTsv, setConvertedTsv] = useState("");
    ...
    const jsonToTsv = (json) => {
        try {
            ...
            if (typeof data === "object" && !Array.isArray(data)) {
                return jsonToTsv(JSON.stringify([data]));
            }
            ...
            const rows = flattenedData.map((obj) =>
                headers.map((header) => {
                    ...
                    if (typeof value === "boolean") {
                        return `${value}`; // No quotes for TSV
                    }
                    return `${value || ""}`; // Handle other values
                }).join("\t") // Use tab as the delimiter
            );
    ...
    const handleInputChange = (e) => {
        ...
        if (!json.trim()) {
            ...
            setErrorDetail("");
            setShowError(false);
            setConvertedTsv(""); // Clear TSV output
            return;
        }
                ...
                <div className="grid grid-cols-2 w-full gap-5">
                    ...
                    <div className="border border-gray-300 rounded-lg overflow-hidden">
                        <div className="bg-gray-800 p-2 flex justify-end space-x-4">
                            ...
                            <span onClick={() => handleCopy(convertedTsv)} className="cursor-pointer text-white">
                                <Copy size={25} />
                            </span>
                        ...
                        <div className="flex">
                            <div
                                ...
                                ref={outputLineRef}
                            >
                                <pre>{getLineNumbers(convertedTsv)}</pre>
                            </div>
                            <textarea
                                ref={outputTextRef}
                                onScroll={handleScroll}
